...
Original file is located at
    https://colab.research.google.com/drive/1UP87F5GfLRDcUAdKTOrWOaeMRbf93WC_
"""

import numpy as np

# Importante: verifica que tu nombre y número de matrícula esten correctos
nombre = "Sebastian Rodriguez Segura"
numero_de_matricula = 175596
fecha = '2025-02-26'
...
def capitalizacion():
    data_capitalizacion = np.array([17, 21, 44, 50, 79, 86, 140, 178, 203])
    media = np.mean(data_capitalizacion)
    mediana = np.median(data_capitalizacion)
    desv_est = np.std(data_capitalizacion)
    tupla_e1 = (media, mediana, desv_est)
    return (tupla_e1)
...
def asistencia_dispersion():
    ...
    tupla_e2 = (rango, varianza, desv_est)
    return (tupla_e2)
...
def histograma_np():
    ...
    histograma = plt.hist(calificaciones, bins=10)
    return histograma
...
def correlacion():
    ...
    y = [1305710, 1658277, 1894167, 2136552, 2298267, 2553624, 2780503, 3289726, 3472743, 3779477]
...
def problema_especifico():
  respuesta = """El problema es que no es posible el analisis de asistencias en una empresa
  """
  return respuesta

def importancia():
  respuesta = """
  Es necesario resolver este problema debido a que en la empresa se encuentran inconsitencias a la hora de registrar las asistencias de los empleados,
  algunos reportan faltas de seguridad saltando las vallas de seguridad, por lo que es necesario identificar al personal que realiza estas acciones
  """
  return respuesta
