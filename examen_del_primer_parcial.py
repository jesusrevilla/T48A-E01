...
Original file is located at
    https://colab.research.google.com/drive/1bsViv9eijiivtZOrMmAW5b_FRaXEdAkU
"""
...
def capitalizacion():
    arreglo = np.array([17, 21, 44, 50, 79, 86, 140, 178, 203])
    media = np.mean(arreglo)
    mediana = np.median(arreglo)
    desv_est = np.std(arreglo)
    return tuple(np.array([media, mediana, desv_est]))
...
def asistencia_dispersion():
    arreglo = np.array([20100, 24500,31600,28400,49500,19350,25600,30600,11300,28560])
    rango = np.max(arreglo) - np.min(arreglo)
    varianza = np.var(arreglo)
    desv_est = np.std(arreglo)
    return tuple(np.array([rango, varianza, desv_est]))

print(asistencia_dispersion())
...
def probabilidad_condicional():
    ...
    return tuple(np.array([p_hombre, p_po_hombre]))
...
def problema_especifico():
    respuesta = """Realizar un analisis de los estudiantes en universidades para definir cuales pueden ser los 'filtros' en las carreras. aquellas materias que
    se llegan a complicar para los estudiantes y ocacionan que estos dejen el estudio."""
    return respuesta

def importancia():
    respuesta = """
    Es importante ya que al obtener está información se puede conocer más a detalle las causas del porque los estudiantes de universidades dejan sus estudios
    """
    return respuesta

def objetivos():
    respuesta = """
    Obtener niveles de aprobación de los estudiantes en las universidades.
    Conocer cuales son las materias más complicadas
    Conocer cuales son las materias más fáciles
    Conocer cuales son las universidades más prometedoras
    Plantear planes de trabajo para las universidades que cuentan con un menor número de estudiantes aprobados
    """
    return respuesta
...
def tipo_de_datos():
    ...
    Indice de reprobados/aprobados
    Universidades más prometedoras
    """
    return respuesta
