...
Original file is located at
    https://colab.research.google.com/drive/1a2gujbHI5t1WVEjIAVqSy6d-dldQVQP9
"""

import numpy as np

# Importante: verifica que tu nombre y número de matrícula esten correctos
nombre = "Aneth Alejandra López Cerda"
numero_de_matricula = 173577
fecha = '2025-02-26'
...
def capitalizacion():
    ...
    moda = np.bincount(datos).argmax()  # Si hay multimodalidad, se debe ajustar
    desv_est = np.std(datos, ddof=0)  # Desviación estándar poblacional
...
def asistencia_dispersion():
    datos = np.array([20100, 24500, 31600, 28400, 49500,
                      19350, 25600, 30600, 11300, 28560])

    rango = np.max(datos) - np.min(datos)
    varianza = np.var(datos, ddof=0)  # Varianza poblacional
    desv_est = np.std(datos, ddof=0)  # Desviación estándar poblacional
...
def histograma_np():
    calificaciones = np.array([7.9, 7.8, 7.8, 6.7, 7.6, 8.7, 8.5, 7.3,
                               6.6, 9.9, 8.4, 7.2, 6.6, 5.7, 9.4, 8.4,
                               7.2, 6.3, 5.1, 4.8, 5.0, 6.1, 7.1, 8.2,
                               9.3, 10.0, 8.9])

    hist, bins = np.histogram(calificaciones, bins=10, range=(4.8, 10.0))
    return (hist, bins)
...
def correlacion():
    ...
    precio = np.array([1305710, 1658277, 1894167, 2136552, 2298267,
                       2553624, 2780503, 3289726, 3472743, 3779477])
...
def probabilidad_condicional():
    total_hombres = 60 + 70
    total_mujeres = 44 + 76
    total_detenidos = total_hombres + total_mujeres

    p_hombre = total_hombres / total_detenidos
    p_po_hombre = 60 / total_hombres  # Probabilidad condicional: primera ofensa dado que es hombre
